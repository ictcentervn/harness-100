...
**스캐폴드 규칙**: 실행 가능한 코드는 문서 내 코드블록만으로 끝내지 않고 `_workspace/src/`에 실파일로 저장한다. `src/`를 생성할 때는 스택에 맞는 verify 진입점을 반드시 포함한다 — Node 스캐폴드면 package.json에 `"verify": "tsc --noEmit"` 스크립트, Python이면 `verify.sh`(`python -m py_compile` + `pytest`), 그 외 스택도 동등한 `verify.sh`를 둔다. 검증 게이트(Phase 3)가 이 진입점을 실행한다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

테스터의 문서 검증과 별개로, 명령을 실제로 실행해 통과를 확인한다. 이 게이트는 `_workspace/src/`에 코드가 생성된 경우에만 적용한다 — 설계 모드·테스트 모드 등 코드 미산출 모드에서는 게이트를 생략하고 최종 보고에 "해당 없음"으로 기록한다.

1. `_workspace/src/` 디렉토리에서 스캐폴드에 포함된 verify 진입점을 실행하고 실제 출력을 확인한다 (Node: `npm run verify` ≒ `npx tsc --noEmit` / Python: `verify.sh` = `python -m py_compile` + `pytest`)
2. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
3. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 대화 설계/NLU 인터페이스 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
4. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
5. 에러 무시 주석 추가, 타입 검사 설정 완화, verify 진입점 약화로 게이트를 우회하지 않는다
...
| 빌드/타입·문법 에러 | 오케스트레이터가 `_workspace/src/`의 verify 진입점 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
