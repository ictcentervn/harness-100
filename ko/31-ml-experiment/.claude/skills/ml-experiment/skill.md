...
**실파일 저장 규칙**: 실행 가능한 코드(전처리·모델·학습·평가 스크립트)는 `.md` 본문의 코드 블록에만 두지 말고 반드시 `_workspace/experiment_code/`에 `.py` 파일로 저장한다. 검증 게이트(Phase 3)가 이 디렉토리를 검사한다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

리뷰어의 문서 리뷰와 별개로, 명령을 실제로 실행해 통과를 확인한다:

1. `_workspace/experiment_code/`에 `.py` 파일이 존재할 때만 게이트를 적용한다. 코드 미산출 런(리뷰 모드·평가 모드 등)에서는 게이트를 생략하고 최종 보고에 "검증 게이트: 해당 없음(코드 미산출)"으로 기록한다
2. `python3 -m compileall -q _workspace/experiment_code/`를 실행하고 실제 출력을 확인한다 — 게이트는 구문 수준 검사로 한정한다. 학습 스모크 실행은 의무화하지 않는다 (torch/xgboost/mlflow 미설치·데이터 부재 환경에서 코드 품질과 무관하게 실패하기 때문)
3. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
4. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 데이터 스키마/모델 아키텍처 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
5. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
6. 문제 파일을 삭제하거나 코드를 `.md` 본문으로 되돌리는 방식으로 게이트를 우회하지 않는다
7. 통과(에러 0건) 또는 "해당 없음" 확인 뒤에만 Phase 4로 진행한다
...
| 코드 구문/임포트 에러 | 오케스트레이터가 `python3 -m compileall -q _workspace/experiment_code/` 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
