...
- **실행 검증 의무**: 보고서 작성 전에 `_workspace/` 루트에서 `npm run verify`(= `npx tsc --noEmit`)를 직접 교차 실행하고 실제 출력을 보고서의 "실행한 검증" 표에 기록한다. 대비비 계산·ARIA 대조·키보드 매트릭스는 코드 분석 기반 **정적 검토**이므로 표에 정적 검토임을 명기한다 — 실행하지 않은 검증을 실행한 것으로 기재하지 않는다
...
`_workspace/04_a11y_report.md` 파일로 저장한다:
    ...
    | 항목 | 방식 | 결과 |
    |------|------|------|
    | npm run verify (tsc --noEmit) | 실행 | [종료 코드 0/1 — 에러 0건 / 에러 N건: 대표 에러 1줄] |
    | 색상 대비비 계산 | 정적 검토 (수치 계산) | [검증한 색상 쌍 N개] |
    | ARIA 패턴 대조 | 정적 검토 (코드 분석) | [대조한 컴포넌트 N개] |
    | 키보드 네비게이션 | 정적 검토 (코드 분석 — 실 브라우저 미실행) | [점검한 컴포넌트 N개] |

    ## 검증 개요
    - **기준**: WCAG 2.1 Level AA
    - **검증 대상**: [컴포넌트 N개]
    - **종합 판정**: PASS / CONDITIONAL PASS / FAIL — 위 "실행한 검증" 표에 근거해서만 판정한다
...
- 컴포넌트 소스 미완성·실파일 미저장 시: FAIL로 보고하고 component-developer에게 반환한다 — 체크리스트만 작성하고 통과로 처리하지 않는다
- 실행 도구 부재 시(axe, 스토리북 a11y 애드온 등): 해당 항목을 통과로 표기하지 않고 "정적 검토로 대체 — 도구 미실행"을 보고서에 명기한다. 실행 가능한 명령(`npm run verify`)만 실행 결과로 기록한다
