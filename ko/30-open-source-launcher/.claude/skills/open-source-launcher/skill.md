...
**스캐폴드 규칙**: 프로젝트에 실제 배치할 파일(CI 워크플로우, 설정 파일, 빌드 매니페스트 등)은 산출 문서(.md) 임베드만으로 끝내지 않고 `_workspace/generated_files/`에 실파일로 저장한다. 빌드 매니페스트(package.json, pyproject.toml 등)를 구성할 때는 test/lint 명령 정의를 반드시 포함하고, 빌드/테스트 명령은 `01_code_organization.md`의 "빌드 명령/테스트 명령" 항목에 기재한다 — Phase 3 검증 게이트가 이 명령을 실행한다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

reviewer의 문서 리뷰와 별개로, 검증 가능한 산출물을 실제로 실행해 통과를 확인한다. 검증 대상은 이원화한다:

1. **생성 파일 문법 검증**: `_workspace/generated_files/` 내 `.github/workflows/*.yml` 등 YAML/TOML 설정 파일을 `actionlint`(없으면 `yamllint`) 등 문법 린트로 검증하고 실제 출력을 확인한다 (exit 0/1 기계 판정)
2. **사용자 코드베이스 검증**: 사용자가 코드베이스를 제공한 경우, `01_code_organization.md`에 기재된 빌드/테스트 명령을 직접 실행하고 실제 출력을 확인한다
3. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
4. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 프로젝트 구조/라이선스 정책 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
5. 검증 도구 부재(actionlint/yamllint 미설치 등) 또는 끝내 통과하지 못한 항목은 TODO.md에 기록하고 최종 보고에 명시한다 — 통과로 표기하지 않는다
6. 워크플로우 파일을 검증 대상에서 제외하거나 린트 무시 주석을 추가하는 방식으로 게이트를 우회하지 않는다
7. **조건부 스킵**: 문서/라이선스/리뷰 모드 등 설정 파일·코드를 산출하지 않는 모드에서는 게이트를 생략하고 최종 보고에 "해당 없음"으로 기록한다
...
| 설정 파일 문법/빌드 에러 | 오케스트레이터가 `actionlint`/`yamllint`와 `01_code_organization.md`의 빌드/테스트 명령을 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
