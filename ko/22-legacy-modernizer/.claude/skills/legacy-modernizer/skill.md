...
4. 대상 코드베이스에 접근 가능하면 **기존 빌드/테스트 명령을 탐지**하여 `_workspace/00_input.md`에 기록한다 (`package.json` scripts, `Makefile`, `pom.xml`, `build.gradle`, CI 설정 등에서 탐지 — Phase 3 검증 게이트가 이 명령을 사용한다). 탐지 불가 시 "검증 명령 미탐지"로 기록한다
...
**실파일 저장 규칙**: 마이그레이션 모드에서 engineer는 변환 코드를 `_workspace/03_migration_plan.md`의 코드블록만으로 산출 종료하지 않고, 대상 프로젝트의 실제 파일에 적용한다. 대상 코드베이스에 쓸 수 없는 경우 `_workspace/migrated_src/`에 실파일로 저장하고 적용 대상 경로를 명시한다 — Phase 3 검증 게이트가 검사할 실물이 있어야 한다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

tester의 회귀 테스트 보고와 별개로, 명령을 실제로 실행해 통과를 확인한다. 이 게이트는 **실제 코드 수정이 발생한 경우**(마이그레이션 모드 등)에만 적용한다. 문서 전용 실행(분석/전략 모드 등)은 게이트를 생략하고 최종 보고에 "검증 게이트 해당 없음(문서 전용)"으로 기록한다.

1. Phase 1에서 `_workspace/00_input.md`에 기록한 대상 프로젝트의 빌드/테스트 명령을 수정된 코드에 대해 실행하고 실제 출력을 확인한다
2. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
3. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 전환 전략·목표 아키텍처 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
4. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
5. 검증 명령이 미탐지이거나 실행 환경이 없으면 통과로 표기하지 않는다 — TODO.md에 기록하고, 정적 분석으로 대체한 경우 종합 판정을 🟢로 표기하지 않는다 (🟡 조건부 + "동적 검증 불가" 명시)
6. 실패 테스트 삭제·주석 처리, 검증 명령 축소, 빌드 설정 완화로 게이트를 우회하지 않는다
7. 통과를 확인한 뒤에만 Phase 4로 진행한다
...
| 테스트 환경 없음 | 정적 분석과 코드 리뷰 기반 검증으로 대체 — 단 종합 판정을 🟢로 표기하지 않는다 (🟡 조건부 + "동적 검증 불가" 명시) |
| 빌드/테스트 에러 | 오케스트레이터가 `00_input.md`에 기록된 빌드/테스트 명령 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
