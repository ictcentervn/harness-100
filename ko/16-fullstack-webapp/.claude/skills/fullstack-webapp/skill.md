...
QA의 문서 리뷰와 별개로, 명령을 실제로 실행해 통과를 확인한다:

1. 앱 디렉토리에서 `npm run verify`(없으면 `npx tsc --noEmit`)를 실행하고 실제 출력을 확인한다
2. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
3. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 스키마/API 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
