...
- **실행 검증 의무**: 산출물 작성 전에 `python3 -m compileall -q _workspace/src`(Node.js 산출로 분기된 경우 `node --check` 또는 `npx tsc --noEmit`)를 직접 실행하고 실제 출력을 산출물에 기록한다. 문서만 읽고 통과 판정하지 않는다. 단, 실행 가능한 명령만 실행·기록한다 — 선택자 대조, 설정 검토 등 정적 검토는 정적 검토임을 명기하고, 실행하지 않은 검증을 실행한 것처럼 기재하지 않는다
...
`_workspace/05_monitor_config.md` 파일로 저장하고, 설정 파일은 `_workspace/src/`에 저장한다:
    ...
    | [정적 검토: 선택자 ↔ 변경 감지 대상 대조 등] | 정적 검토 | [실행 없는 검토임을 명기] |
...
- `_workspace/src/` 코드 미완성·구문 에러 시: 통과로 처리하지 않고 해당 팀원에게 반환한다 — 모니터링 설정만 작성하고 시스템이 검증된 것처럼 보고하지 않는다
- 검증 명령을 실행할 수 없는 환경: 실행 불가 사실과 사유를 산출물에 기록하고 정적 검토 결과는 정적 검토로만 명기한다 — 통과로 표기하지 않는다
