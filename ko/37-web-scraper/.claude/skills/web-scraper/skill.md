...
**실파일 저장 규칙**: 실행 가능한 코드는 `.md` 문서의 코드블록·의사코드로만 남기지 말고 반드시 `_workspace/src/`에 실파일(`.py`)로 저장한다. 검증 게이트(Phase 3)는 이 디렉토리의 실파일을 대상으로 실행된다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

모든 에이전트의 작업이 완료되면 통합하고, 명령을 실제로 실행해 통과를 확인한다:

1. `_workspace/` 내 모든 파일과 `src/` 코드를 확인한다
2. 컴포넌트 간 인터페이스 정합성을 검증한다
3. 실행 가능한 통합 스크립트(`_workspace/src/main.py`)를 생성한다
4. `python3 -m compileall -q _workspace/src`를 실행하고 실제 출력을 확인한다 (Node.js 산출로 분기된 경우 `node --check` 또는 `npx tsc --noEmit`)
5. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
6. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 크롤러 아키텍처/데이터 스키마 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
7. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
8. 에러 나는 파일의 삭제·주석 처리나 검사 대상 축소로 게이트를 우회하지 않는다
9. 통과(에러 0건)를 확인한 뒤에만 Phase 4로 진행한다

분석 모드 등 코드를 산출하지 않는 모드에서는 게이트를 생략하고 "해당 없음"으로 기록한다. 코드 산출 모드에서 `_workspace/src/`가 비어 있으면 생략이 아니라 미통과로 처리한다.

### Phase 4: 최종 보고

검증 게이트 통과 후 최종 요약을 사용자에게 보고한다:
...
| 구문/임포트 에러 | 오케스트레이터가 `python3 -m compileall -q _workspace/src` 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
