...
**실파일 저장 규칙**: 코드 생성 에이전트(etl-architect, scheduler-engineer, data-quality-manager)는 실행 가능한 Python 코드(Airflow DAG, 검증 스크립트 등)를 .md 산출물에 임베드하는 것과 별개로 `pipeline_code/`에 .py 실파일로 저장한다. 문서 내 코드블록만으로 산출을 종료하지 않는다. (dbt SQL은 dbt 프로젝트 부재로 기계 검증 대상에서 제외)

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

리뷰어의 문서 교차 검증과 별개로, 명령을 실제로 실행해 통과를 확인한다:

1. `pipeline_code/`에 .py 파일이 있으면 `find pipeline_code -name '*.py' -exec python3 -m py_compile {} +`를 실행하고 실제 출력을 확인한다 (dbt SQL은 dbt 프로젝트 부재로 기계 검증 제외)
2. 코드 미산출 모드(모니터링 모드·리뷰 모드 등)에서 코드 파일이 없으면 "게이트 해당 없음"으로 기록하고 건너뛴다. 단, 코드 산출 모드에서 `pipeline_code/`가 비어 있으면 미통과로 처리한다 (파일 부재를 스킵 사유로 삼지 않는다)
3. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
4. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 스키마/DAG 구조 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
5. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
6. 에러 나는 파일을 `pipeline_code/`에서 제거하거나 코드를 .md 임베드로만 남기는 방식으로 게이트를 우회하지 않는다
7. 통과(에러 0건 또는 게이트 해당 없음 확인)를 확인한 뒤에만 Phase 4로 진행한다
...
| 코드 구문 에러 | 오케스트레이터가 `python3 -m py_compile` 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
