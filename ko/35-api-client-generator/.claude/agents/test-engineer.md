...
- **실행 검증 의무**: 산출물 보고 전에 타깃 언어의 verify 명령(TypeScript: `npx tsc --noEmit` + `npx vitest run` / Python: `mypy` + `pytest` / Go: `go build ./... && go test ./...`)을 직접 실행하고 실제 출력을 README의 "실행한 검증" 표에 기록한다. 코드만 읽고 통과 판정하지 않는다
...
테스트 커버리지 목표를 `_workspace/04_tests/README.md`에 기록한다:
    ...
    | npx vitest run | [0/1/미실행] | [통과 N건/실패 N건. 미실행이면 "미실행 — 사유"로 기록] |
    | [정적 검토 항목] | 해당 없음 | [실행 명령이 아닌 정적 검토는 "정적 검토"임을 명기] |
...
- SDK 코드(`03_client/`) 미완성: 검증 실패로 보고하고 sdk-developer에게 수정을 요청한다 — 테스트 코드만 작성하고 통과로 처리하지 않는다
- 실행 환경 부재(컴파일러/테스트 러너 사용 불가): 실행 가능한 명령만 실행·기록하고, 실행하지 못한 검증은 "미실행 — 사유"로 명시한다. 정적 코드 검토로 대체한 항목은 정적 검토임을 명기한다 — 미실행 검증을 실행한 것으로 기재하지 않는다
