...
**실파일 저장 규칙**: 실행 가능한 코드는 `_workspace/src/`에 실파일로 저장한다. 문서 내 코드블록·의사코드만으로 산출을 종료하지 않는다 — 검증 게이트(Phase 3)가 검사할 실물이 있어야 한다.

### Phase 3: 검증 게이트 (오케스트레이터가 메인 컨텍스트에서 직접 수행 — 서브에이전트 위임 금지)

에이전트의 문서 산출과 별개로, 명령을 실제로 실행해 통과를 확인한다:

1. `python3 -m compileall -q _workspace/src`를 실행하고 실제 출력을 확인한다 (의존성 설치 불요 — 문법만 검사). `src/`에 package.json이 존재하면 `npm install && npx tsc --noEmit`(TypeScript) 또는 `node --check`(JavaScript)로 전환한다. Dockerfile/YAML은 게이트 범위에서 제외한다
2. 실패 시 에러를 직접 수정하고 재실행한다 — 통과할 때까지 반복
3. 동일 에러가 3회 반복되면 접근을 바꾼다. 수정이 프롬프트/RAG 파이프라인 등 설계 변경을 요구하면 자동 수정하지 말고 사용자에게 보고한다
4. 끝내 통과하지 못하면 잔여 에러를 TODO.md에 기록하고 최종 보고에 명시한다
5. 에러 나는 파일을 `src/`에서 삭제하거나 코드를 문서 코드블록으로 되돌리는 방식으로 게이트를 우회하지 않는다
6. 통과(에러 0건)를 확인한 뒤에만 Phase 4로 진행한다

**스킵 조건**: 프롬프트 모드 등 문서 전용 실행은 게이트를 스킵한다. 코드 산출 모드(풀/RAG/평가/최적화/배포)에서 `_workspace/src/`가 비어 있으면 미통과로 처리한다 — 파일 부재를 스킵 사유로 삼지 않는다. `src/`에 코드가 실제 산출된 경우에는 모드와 무관하게 게이트를 적용한다.

### Phase 4: 통합 및 최종 산출물

1. 평가 메트릭이 기준을 충족하는지 확인한다
2. 배포 설정이 완전한지 검증한다
3. 최종 요약을 사용자에게 보고한다:
...
| 빌드/문법 에러 | 오케스트레이터가 `python3 -m compileall -q _workspace/src` 직접 실행 → 에러 분석 → 수정 → 재검증 (Phase 3 게이트) |
